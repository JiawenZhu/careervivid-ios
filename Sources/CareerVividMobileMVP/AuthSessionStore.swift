...
final class AuthSessionStore: ObservableObject {
    ...
    func load() async {
        print("DEBUG: AuthSessionStore.load() started")
        isLoading = true
        print("DEBUG: AuthSessionStore.load() calling currentSession()")
        session = await CVFirebaseAuth.shared.currentSession()
        print("DEBUG: AuthSessionStore.load() currentSession() returned: \(String(describing: session))")
        isLoading = false
        print("DEBUG: AuthSessionStore.load() finished")
    }
