...
extension Color {
    static func cvDynamic(light: Color, dark: Color) -> Color {
        #if os(iOS)
        // Convert Colors to UIColors statically outside the dynamic trait provider block
        // using safeUIColor to prevent thread deadlocks/layout loops during launch.
        let uiLight = light.safeUIColor
        let uiDark = dark.safeUIColor
        return Color(UIColor { traitCollection in
            traitCollection.userInterfaceStyle == .dark ? uiDark : uiLight
        })
        #else
        return light
        #endif
...
extension Color {
    ...
    static let cvCardBorder = Color.cvDynamic(
        light: Color(white: 1.0, opacity: 0.85),
        dark: Color(red: 0.173, green: 0.173, blue: 0.180, opacity: 0.35)
    )
    ...
    static var cvTertiarySystemBackground: Color {
        Color.cvDynamic(
            light: Color(red: 0.976, green: 0.976, blue: 0.984),
            dark: Color(red: 0.173, green: 0.173, blue: 0.180)
        )
    }
    ...
    static var cvSystemFill: Color {
        Color.cvDynamic(
            light: Color(red: 0.910, green: 0.910, blue: 0.925),
            dark: Color(red: 0.141, green: 0.141, blue: 0.149)
        )
    }
        ...
        content
            ...
            .overlay(
                RoundedRectangle(cornerRadius: radius, style: .continuous)
                    .stroke(Color.cvCardBorder, lineWidth: 1)
            )
            .shadow(
