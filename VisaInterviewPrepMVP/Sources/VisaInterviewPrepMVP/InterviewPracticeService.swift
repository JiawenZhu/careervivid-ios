...
struct InterviewLiveConfig: Equatable, Sendable {
    ...
    var prompt: String {
        ...
        if isVisa {
            return """
            U.S. Visa Interview for \(jobTitle) conducted in \(language). Sponsoring/Inviting entity or Consulate context: \(job.company).
            Evaluate the candidate's responses as a mock visa applicant answering a consular officer.
            Key criteria:
            1. Non-immigrant intent / ties to home country (crucial for B1/B2, F-1, J-1).
            2. Financial viability and clear funding details.
            3. Clarity, honesty, and consistency.
            4. Brevity: Consular interviews are brief (2-5 minutes). Do NOT penalize the candidate for giving short, direct, 1-3 sentence answers. In fact, favor concise and clear answers over long-winded paragraphs.
            """
        } else {
            let base = "\(category.rawValue) interview for \(job.title) at \(job.company). Focus on role-specific proof, communication, decision quality, and practical experience. Conducted in \(language)."
